...
import { parseImportFile, confirmImport, downloadAsFile } from '../services/api';
...
export default function ImportBooksModal({ onClose, onImportComplete }) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (booksToImport.length === 0) {
        setError('No books selected for import');
        setStep(STEPS.REVIEW);
        return;
      }

      const results = await confirmImport(booksToImport);
      setImportResults(results);
      setStep(STEPS.DONE);

      if (results.imported > 0) {
        onImportComplete(results.books);
      }
    } catch (err) {
      setError(err.message);
      setStep(STEPS.REVIEW);
  ...
  const handleExportSkipped = () => {
    ...
    parseResults.results.duplicates.forEach(item => {
      skippedBooks.push({
        ...
        author: item.original.author,
        isbn: item.original.isbn,
        reason: 'Duplicate - already in library'
      });
    });
    ...
    downloadAsFile(csv, `skipped-books-${new Date().toISOString().split('T')[0]}.csv`, 'text/csv');
  };
    ...
    <>
      ...
      {parseResults.results.found.length > 0 && (
        <div className="mb-4">
          ...
          <p className="text-xs text-theme-muted mb-2">
            ISBN found—these books will be imported with full metadata (synopsis, page count, series info).
          </p>
          <details className="bg-green-50 border border-green-200 rounded-lg">
            <summary className="p-3 cursor-pointer text-sm text-green-800 hover:bg-green-100 rounded-lg">
              Show {parseResults.results.found.length} books to import
            </summary>
      ...
      {parseResults.results.notFound.length > 0 && (
        <div className="mb-4">
          ...
          <p className="text-xs text-theme-muted mb-3">
            No ISBN found for these books. Select which ones to add with only the basic info from your file (no additional metadata).
          </p>
      ...
      {parseResults.results.duplicates.length > 0 && (
        <div className="mb-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
          <p className="text-sm text-gray-700 mb-2">
            <span className="font-medium">{parseResults.results.duplicates.length} duplicate books</span> already in your library will be skipped.
          </p>
      ...
      <div className="p-3 bg-theme-secondary rounded-lg mb-4">
        <p className="text-sm text-theme-primary">
          Ready to import <span className="font-bold">{getTotalToImport()} books</span>
        </p>
      </div>
  ...
  const renderImportingStep = () => (
    <div className="text-center py-8">
      ...
      <p className="text-theme-primary font-medium">Importing books...</p>
      <p className="text-sm text-theme-muted mt-2">Adding books to your library</p>
    </div>
  );
  ...
  const renderDoneStep = () => (
    <div>
      <div className={`p-4 rounded-lg mb-4 ${
        importResults.imported > 0 ? 'bg-green-100 border border-green-400' : 'bg-yellow-100 border border-yellow-400'
      }`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className={`font-medium ${importResults.imported > 0 ? 'text-green-800' : 'text-yellow-800'}`}>
      ...
      <div className="bg-theme-card rounded-lg p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-theme-primary">
            {step === STEPS.UPLOAD && 'Import Books'}
